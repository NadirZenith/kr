...
// Send an API request
const rawRequest = (url, headers, data, timeout) => {
    ...
    publicMethod(method, params, callback) {
        ...
        if (typeof callback === 'function') {
            response
                .then((rsp) => {
                    if (rsp.status === 200) {
                        // console.log('kraken client1', rsp)
                        callback(null, rsp.data)
                    } else {
                        // console.log('kraken client2', rsp)
                        callback(rsp.data, null)
                    }
                })
                .catch((rsp) => callback(rsp.error, null))
        }
